...
const Forum = () => {
    ...
    const { handleAddPost, forumData, handleUpvote, handleDownvote } = useContext(PageContext);
                    ...
                    {filteredData.length > 0 ? filteredData.map((post, index) => (
                        ...
                        >
                            <div className="forum-card-votes">
                                <button className="vote-btn" onClick={() => handleUpvote(post._id)}><ArrowBigUp size={24} /></button>
                                <span>{post.upvotes - post.downvotes}</span>
                                <button className="vote-btn" onClick={() => handleDownvote(post._id)}><ArrowBigDown size={24} /></button>
                            </div>

                            <div className="forum-card-content">
                                <Link to={`/forum/topic/${post._id}`}>
                                    <h3>{post.title}</h3>
                                </Link>
